...
func (adm *AgentGormRepo) Agent(agentData *entity.Agent) (*entity.Agent, []error) {
	...
	//agent := entity.Agent{}
	//errs := adm.conn.Where("email = ? AND password = ?", agentData.Email, agentData.Password).First(&agent).GetErrors()
	//if len(errs) > 0 {
	//	return nil, errs
	//}
	//return &agent, errs
	...
	//	return nil, errs
	//}
	//same := handler.VerifyPassword(adminData.Password, agent.Password)
	//fmt.Println("is same",same)
	//if same {
	//	errs := adm.conn.Where("email = ?", adminData.Email).First(&agent).GetErrors()
	//	return &agent, errs
	//}
}
